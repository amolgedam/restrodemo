...
class Model_reports extends CI_Model
{
	...
	{
		$query = $this->db->select('products.name as product_name, orders.net_amount, orders.discount, orders.totalgst, orders.gross_amount')
							->from('orders')
							->where('orders.date_time =', $day)
							->join('stores', 'stores.id = orders.store_id', 'left')
							->join('products', 'products.id = stores.id', 'left')
							->get();
		return $query->result();
	}
